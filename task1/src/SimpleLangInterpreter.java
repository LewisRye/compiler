...
public class SimpleLangInterpreter extends AbstractParseTreeVisitor<Integer> implements SimpleLangVisitor<Integer> {
    ...
    @Override
    public Integer visitReassignExpr(SimpleLangParser.ReassignExprContext ctx) {
    ...
    public Integer visitWhileExpr(SimpleLangParser.WhileExprContext ctx) {
        ...
        while (condValue > 0) {
            ...
            SimpleLangParser.BlockContext statement = ctx.block();
            for (int i = 0; i < statement.ene.size(); i++) {
                visit(statement.ene.get(i));
            }
